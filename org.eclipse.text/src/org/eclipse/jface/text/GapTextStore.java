 ...
 * location of the last change. Typing-style changes perform in constant time unless re-allocation
 * becomes necessary. Generally, a change that does not cause re-allocation will cause at most one
 * {@link System#arraycopy(Object, int, Object, int, int) arraycopy} operation of a size of about
 * <em>d</em>, where <em>d</em> is the distance from the previous change. Assuming that
 * <code>arraycopy</code> performs in <em>O(d)</em> (with a high constant factor), such a change
 * also performs in <em>O(d)</em>.
 * <p>
...
public class GapTextStore implements ITextStore {
	/*
	 * Flag for debug output. Set to false.
	 * @since 3.3.
	 */
	 ...
	 * allocated gap size. Use <code>minSize</code> to avoid frequent re-allocation for very small
	...
	public GapTextStore(int minSize, int maxSize, float maxGapFactor) {
		Assert.isTrue(0f <= maxGapFactor && maxGapFactor <= 1f);
		Assert.isTrue(0 <= minSize && minSize <= maxSize);
		fMinGapSize= minSize;
		fMaxGapSize= maxSize;
	...
	private int reallocate(int offset, int remove, final int oldGapSize, int newGapSize, final int newGapStart) {
		...
		final char[] newContent= new char[newArraySize];
		final int newGapEnd= newGapStart + newGapSize;

		if (DEBUG) System.out.println(System.identityHashCode(this) + "\t re-allocated " + (newArraySize > 1024 ? (newArraySize / 1024) + " KB" : newArraySize + " Bytes")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
